class BallOne {
    ...
    draw(ctx) {
        ...
        // Draw body awareness
        ctx.lineWidth = (this._horzHit * 2) * (this._game._entities[3]._bodyAwareness / 100);
        ctx.strokeStyle = 'rgba(250, 255, 5, 0.69)';
        ctx.beginPath();
        ctx.moveTo(this._x, this._y - (this._vertHit * (this._game._entities[3]._bodyAwareness / 100)));
        ctx.lineTo(this._x, this._y + (this._vertHit * (this._game._entities[3]._bodyAwareness / 100)));
        ctx.stroke();
    }
    ...
    update() {
        ...
        if (this._y - this._vertHit < 0 || this.hit()) {
            if (this.hit()) {
                ...
                this._game._entities[3]._nextBall = !this._game._entities[3]._nextBall;
            }
